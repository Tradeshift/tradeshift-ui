...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		browserify: {
			langneg: {
				src: [],
				dest: 'temp/g11n-langneg.js',
				options: {
					require: ['@tradeshift/g11n-langneg']
				}
			}
		},
		...
		concat: {
			...
			// generates a callable object containing a loader for formatting data ("formatDataLoader")
			// for distinct locales and a closure that performs language negotiation and executes it
			locales: {
				src: 'src/runtime/js/ts.ui/lang/*.js',
				dest: 'temp/locales.js',
				options: {
					banner: `var formatDataLoader = {\n`,
					process: getlocalesprocessor(langcodes),
					footer: `\n};`
				}
			},
			// Uses the browserify's `require` to load `g11n-langneg` in runtime.
			// Performs language negotiation and loads formatting data from
			// the object `formatDataLoader`, a variable generated by the Grunt task `concat:locales`.
			matching: {
				src: 'src/runtime/js/ts.ui/lang/matching/matching.js',
				dest: 'temp/matching.js'
			},
	...
	function build(target = 'cdn') {
		const out = [
			...
			'concat:matching', // bundle snippet to perform language matching for `concat:locales`
	...
	/**
	 * Transform JS inside some file into an object,
	 * keyed by locale and valued by a function what loads formatting that for that locale,
	...
	function getlocalesprocessor(codes) {
		return (src, file) => {
			...
			try {
				...
				grunt.fail.fatal(`Invalid locale could not be parsed (${code}): ${e}`);
			}
			...
			return `\t '${locale.toString()}': function () { ${lines} },`;
		};
	}
