 ...
 */
ts.ui.ToolBarSpirit = (function using(chained, confirmed, Client, Type, guiArray, CSSPlugin, DOMPlugin, TextModel, SearchModel, TopBar, PagerModel, bgcolors) {
	...
	return ts.ui.BarSpirit.extend({
		...
		macro: chained(function(){
			this.css.remove(ts.ui.CLASS_MICRO);
			this.css.add(ts.ui.CLASS_MACRO);
		}),
		...
		micro: chained(function(){
			this.css.remove(ts.ui.CLASS_MACRO);
			this.css.add(ts.ui.CLASS_MICRO);
		}),
		 ...
		 * @param @optional {object|ts.ui.PagerModel} opt_json
		 * @returns {ts.ui.PagerModel|ts.ui.ToolBarSpirit}
		 */
		pager: confirmed('(object)')(
			chained(function(opt_json) {
				var model = this.model();
				if (arguments.length) {
					if (model.pager) {
		 ...
		 */
		compact: chained(function(compact) {
			var model = this.model();
			if(arguments.length) {
		...
		_layoutmain: function(attaching) {
			if(!this.dom.ancestor(ts.ui.SideShowSpirit)) { // not for embedded bars...
				if(this.guilayout.beforeMain()) {
					this.css.shift(attaching, 'ts-toolbar-first');
					var micro = this.css.contains(ts.ui.CLASS_MICRO);
					var klass = 'ts-has-toolbar-first-' + (micro ? 'ts-micro' : 'ts-macro');
					this.guilayout.shiftGlobal(attaching, klass);
					if (attaching) {
						this._initbreakpoint();
					}
				} else if(this.guilayout.afterMain()) {
					this.css.shift(attaching, 'ts-toolbar-last');
					this.guilayout.shiftGlobal(attaching, 'ts-has-toolbar-last');
					if (attaching) {
						(function some_refactoring_needed() { // DOH!
							this._looknormal(this.css);
		...
		_cnames: function(small, lefts, right, extra) {
			...
			var status = this.css.contains('ts-statusbar');
			var args = [this.css, small, lefts.length, right.length, extra.length, search];
			this._regularcnames.apply(this, args);
			if(status) {
				if (small) {
					this._specialcnames.apply(this, args);
				} else {
					this._gotolevel(1);
				}
			}
		 ...
		 * @param {truthy} search
		 */
		_regularcnames: function(css, small, lefts, right, extra, search) {
		...
		/**
		 * Special styling hooks for statusbars in mobile breakpoint.
		 ...
		 * @param {truthy} search
		 */
		_specialcnames: function(css, small, lefts, right, extra, search) {
			if (small) {
				this._gotolevel(
					this._computelevel(lefts, right, extra, search)
				);
			}
		},

		/**
		 * Update `ts-level-x` classname and 
		 * dispatch an action if it changed.
		 * @param {number} level
		 */
		_gotolevel: function(level) {
			var css = this.css, cname = css.name();
		...
		/**
		 * Hide tabs that won't fit (and show the More-tab).
		 * TODO: Need to update this for *mobile* breakpoint!
		 * @param {Array<ts.ui.TabModel>} tabs
		*/
			...
			if (moretab && tabs && tabs.getLength()){
				...
				if(!dofit && width < 200) {
					...
					if((gonetab = tabs.find(function ishidden(tab) {
						...
						//this.script.run(); // don't wait for lazy run @todo ask moth why need this one.
					}
				}
			}
		},
		...
		_getavailwidth: function() {
			...
			return this.box.width - (right ? right.offsetWidth : 0) - (center ? center.offsetWidth : 0);
		},
